...
/* copy from wx for internal support, no need wx library */
...
namespace GenUtils {
	...
	// other
	string Pad(const string& s, int width, bool pad_left=true);
	string DblToStr(double x, int precision = 3);
	string PtToStr(const wxPoint& p);
	string PtToStr(const wxRealPoint& p);
	...
	string swapExtension(const string& fname, const string& ext);
	string GetFileDirectory(const string& path);
	string GetFileName(const string& path);
	string GetFileExt(const string& path);
	string GetTheFileTitle(const string& path);
    ...
    */
	void strToInt64(const char *str, int64_t *val);
	void strToInt64(const string& str, int64_t *val);
	bool validInt(const char* str);
	bool validInt(const string& str);
	bool isEmptyOrSpaces(const char *str);
	bool isEmptyOrSpaces(const string& str);
	/*
	bool ExistsShpShxDbf(const string& fname, bool* shp_found,
