...
iShapeFile::iShapeFile(const string& fname, const string& ext)
	: std::ifstream(), record(0)
{
	open(GenUtils::swapExtension(fname, ext).c_str(),
...
oShapeFile::oShapeFile(const string& fname, const string& ext)
	: std::ofstream()  
{
    open(GenUtils::swapExtension(fname, ext).c_str(),
