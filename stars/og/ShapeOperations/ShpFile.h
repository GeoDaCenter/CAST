...
namespace Shapefile {
	...
	struct Header {
		...
		int file_code; // byte 0, BE, should have value 9994
		int file_length; // byte 24, BE, the num of 16-bit words in the file
		int version; // byte 28, LE, should have value 1000
		int shape_type; // byte 32, one of 14 values from ShapeType enum
		double bbox_x_min; // byte 36, LE, bounding box for X. similar below
		double bbox_y_min; // byte 44, LE
		double bbox_x_max; // byte 52, LE
		double bbox_y_max; // byte 60, LE
		double bbox_z_min; // byte 68, LE, if unused, then has value 0.0
		double bbox_z_max; // byte 76, LE, if unused, then has value 0.0
		double bbox_m_min; // byte 84, LE, if unused, then has value 0.0
		double bbox_m_max; // byte 92, LE, if unused, then has value 0.0
	};
	
	enum ShapeType {
		NULL_SHAPE = 0, POINT = 1, POLY_LINE = 3, POLYGON = 5,
	...
	std::string shapeTypeToString(int st);
	...
	struct PointContents : public RecordContents {
		PointContents() : RecordContents(Shapefile::POINT), x(0), y(0) {}
	...
	double myDOUBLE_SWAP_ON_BE( double x );
	int myINT_SWAP_ON_BE( int x );
	int myINT_SWAP_ON_LE( int x );
	...
	std::string boxToString(const std::vector<double>& box);
