					 ...
					 std::ifstream& file)
{
  bool success = true;
  int integer32;
  int* integer32p = &integer32;
  double float64;
  double* float64p = &float64;
...
std::string Shapefile::boxToString(const std::vector<double>& box)
{
										...
										Index& index, std::ifstream& main_f)
{
	...
	*/
	...
	main_f.read((char*) integer32p, 4);
	int m_content_length = myINT_SWAP_ON_LE(integer32);
...
double Shapefile::myDOUBLE_SWAP_ON_BE( double x )
{
  	if is_bigendian() {
		char* c = (char*) &x;
		union {
