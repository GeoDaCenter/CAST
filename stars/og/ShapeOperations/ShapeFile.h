...
public:
	iShapeFile(const string& fname, const string& ext);
	virtual ~iShapeFile();
	...
	iShapeFile& operator>>(int &v)  // reads 4 byte integer // MMM: big assumption!
    { read((char*) &v, 4); return *this; }
...
public:
	oShapeFile(const string& fname, const string& ext);
	virtual ~oShapeFile();
	oShapeFile& operator<<(const int& v)   // write 4 byte integer
