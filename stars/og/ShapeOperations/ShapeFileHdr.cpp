...
ShapeFileHdr::ShapeFileHdr(const char* s)
{
	...
	FileLength = GenUtils::Reverse(hr->f[6]);
        ...
        int x = GenUtils::ReverseInt(hr64->f[6]);
        LOG(x);
#endif
#ifdef WORDS_BIGENDIAN
...
void ShapeFileHdr::SetFileLength(int fl) 
{
	FileLength = fl;
}
...
void ShapeFileHdr::Replace (const string& fname, const int& recs)  
{
	...
	// update *.shp file
	FILE *shp = fopen(
		GenUtils::swapExtension(fname, "shp").c_str(), "rb+");
	...
	// update *.shx file
	FILE *shx = fopen(
		GenUtils::swapExtension(fname, "shx").c_str(), "rb+");
#ifdef WORDS_BIGENDIAN
	...
	// update *.dbf file
	FILE *dbf = fopen(
		GenUtils::swapExtension(fname, "dbf").c_str(), "rb+");
	int nrec;
