...
string GenUtils::Pad(const string& s, int width, bool pad_left)
{
...
string GenUtils::DblToStr(double x, int precision)
{
	std::stringstream ss;
...
string GenUtils::PtToStr(const wxPoint& p)
{
	std::stringstream ss;
	ss << "(" << p.x << "," << p.y << ")";
	return ss.str();
...
string GenUtils::PtToStr(const wxRealPoint& p)
{
	std::stringstream ss;
...
string GenUtils::swapExtension(const string& fname, const string& ext)
{
	if (ext.length()==0) return fname;
	string prefix = fname.substr(0,fname.find_last_of('.'));
	if (prefix.length()==0) return fname + "." + ext;
	return prefix + "." + ext;
}

string GenUtils::GetFileDirectory(const string& path)
{
...
string GenUtils::GetFileName(const string& path)
{
	int pos = path.find_last_of('/');
	if (pos >= 0)
...
string GenUtils::GetFileExt(const string& path)
{
	int pos = path.find_last_of('.');
	if (pos >= 0)
		return path.substr(pos+1);
...
string GenUtils::GetTheFileTitle(const string& path)
{
	string strResult = GetFileName(path);
	int pos = strResult.find_last_of('.');
	if (pos >= 0)
		return strResult.substr(0,pos);
	return strResult;
}
...
void GenUtils::strToInt64(const string& str, int64_t *val)
{
	char buf[1024];
...
void GenUtils::strToInt64(const char *str, int64_t *val)
{
	int64_t total = 0;
	bool minus = 0;
...
bool GenUtils::validInt(const string& str)
{
	char buf[1024];
	strcpy( buf, (const char*)str.c_str() );
	return validInt(buf);
...
bool GenUtils::isEmptyOrSpaces(const string& str)
{
	char buf[1024];
	strcpy( buf, (const char*)str.c_str() );
