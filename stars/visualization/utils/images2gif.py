...
def writeBmps2Gif(filename, bmps, duration=0.1, loops=0, dither=1):
    images = []
    for bmp in bmps:
        im = bitmapToPil(bmp).convert('P',dither=dither)
        images.append(im)
    ...
    if hasattr(duration, '__len__'):
        ...
        durations = [duration for im in images]
    ...
    # write
    try:
        n = _writeGifToFile(fp, images, durations, loops)
        #print n, 'frames written'
    finally:
        fp.close()
