...
__all__ = ['SigTrendGraphLISA','SigTrendGraphLISAQueryDialog','ShowSigTrendGraphLISA']
...
class SigTrendGraphLISA(PlottingCanvas):
    """
    """
...
Details: """+str(err.message))
    ...
    def DoDraw(self,dc):
        super(SigTrendGraphLISA, self).DoDraw(dc)
        
        # draw y axis at each time interval
        dc.SetFont(wx.Font(self.font_size_y_axis,wx.ROMAN,wx.NORMAL,wx.NORMAL))
    ...
    def plot_data(self,dc):
        ...
        dc.SetFont(wx.Font(self.font_size_y_axis,wx.ROMAN,wx.NORMAL,wx.NORMAL))
        if os.name == 'posix':
            dc.SetFont(wx.Font(self.font_size_y_axis+2,wx.ROMAN,wx.NORMAL,wx.NORMAL))
        self.line_marker = []
        self.verticle_lines = []
        for i in range(self.x_min, self.x_max+1):
            if i == self.x_min or i == self.x_max:
        ...
        # draw legend
        dc.SetFont(wx.Font(stars.LISA_SPACE_LEGEND_FONT_SIZE,wx.ROMAN,wx.NORMAL,wx.NORMAL))
        if os.name == 'posix':
            dc.SetFont(wx.Font(stars.LISA_SPACE_LEGEND_FONT_SIZE+2,wx.ROMAN,wx.NORMAL,wx.NORMAL))
        dc.SetPen(wx.BLACK_PEN)
        y = self.bufferHeight - 14
        x = 1
        colors = [wx.Color(180,180,180),stars.LISA_HH_COLOR,stars.LISA_LL_COLOR,stars.LISA_LH_COLOR, stars.LISA_HL_COLOR,stars.LISA_OBSOLETE_COLOR]
        labels = ["Not significant","High-High","Low-Low","Low-High","High-Low","Neighborless"]
            ...
            if self.selected_line != None:
                ...
                for key in self.data.keys():
                    ...
                    for i in range(t):
                        ...
                        cluster = clusterFlag[i]
                        if cluster == 0:
                            color = wx.Color(220,220,220)#stars.LISA_NOT_SIG_COLOR
                        elif cluster == 1:
                            color = stars.LISA_HH_COLOR
                        elif cluster == 2:
                            color = stars.LISA_LL_COLOR
...
class SigTrendGraphLISAQueryDialog(DynamicLISAQueryDialog):
    """
    ...
    if dlg.ShowModal() == wx.ID_OK:
        ...
        if shp.shape_type == stars.SHP_POINT:
            ...
            gi_dlg = SigTrendGraphLISAQueryDialog(
                self,"LISA Trend Graph:" + shp.name,
                shp, 
